...
# ── Render ────────────────────────────────────────────────────────────────────
def render_cards(data, page_key="cards_shown"):
    if data.empty:
...
for i, tab in enumerate(top_tabs):
    with tab:
        ...
        if cat == "":
            ...
            render_cards(filtered, page_key="cards_all")
            ...
            with nav_col:
                ...
                for item in ["All sports"] + sports_present:
                    ...
                    if st.button(f"{item}", key=f"sp__{item}"):
                        ...
                        st.session_state["cards_shown"] = 20
                        st.rerun()
            ...
            with card_col:
                ...
                render_cards(view, page_key="cards_sport")

        else:
            render_cards(filtered[filtered["category"]==cat].copy(), page_key=f"cards_{cat}")
